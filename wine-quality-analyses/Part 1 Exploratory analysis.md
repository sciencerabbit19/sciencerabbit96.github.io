...
# Draw a heatmap using the full correlation matrix
sns.heatmap(
    df.corr(),
    cmap='viridis',
...
We can observe some important correlations. Since we are interested in wine quality, these relationships can already provide useful clues. For instance, alcohol content correlates positively with wine quality ($\rho = 0.48$), whereas volatile acidity shows a negative correlation with it ($\rho = -0.39$).  To identify outliers, we can use the Isolation Forest algorithm. This algorithm detects outliers in a non-parametric manner:
...
# Feature matrix (remove the target column 'quality')
...
# Store predictions (-1 = outlier, 1 = inlier) in a new column
df['outliers'] = clf.predict(X_scaled)

# Show the labels and their counts
...
```python
# Function to classify a wine sample based on its quality score and outlier flag
def my_func(a, b):
    """
    Classify a wine sample according to its quality score (a) and outlier flag (b).
...
# Apply the classification function row-wise and store the result
...
```python
# Standardise the numerical features (excluding the target and label columns)
from sklearn.preprocessing import StandardScaler
scaler = StandardScaler()

# Select only the numerical features to be scaled
features_to_scale = df.drop(['quality', 'outliers', 'quality_cat'], axis=1)
...
```python
# Custom scatter plot function used inside the PairGrid
def scatter_custom(x, y, alpha=0.3, alpha_out=0.6, **kwargs):
    """
    Custom scatter plot that draws inlier points semi-transparent
    and outliers with higher opacity and a different marker.
    """
    # Extract the DataFrame passed via the `data` argument
    df = kwargs.pop("data")

    # Boolean masks for outliers (-1) and inliers (1)
    mask_out = df["outliers"] == -1
    mask_in  = df["outliers"] == 1

    markers_map = {1: "o", -1: "X"}

    # Inlier points (more transparent)
    sns.scatterplot(
        x=x[mask_in],
        y=y[mask_in],
        alpha=alpha,
    ...
    # Outlier points (less transparent, emphasised)
    sns.scatterplot(
        x=x[mask_out],
        y=y[mask_out],
        alpha=alpha_out,
...
# ----------------------------------------------------------------------
# Create a PairGrid visualising all feature pairs, coloured by quality_cat
# ----------------------------------------------------------------------
plt.figure(dpi=300)

g = sns.PairGrid(
    df_to_plot.drop("outliers", axis=1),   # exclude the raw outlier flag from the grid
    hue="quality_cat",
...
# Off-diagonal: custom scatter showing inliers/outliers
...
This plot supports our hypothesis that alcohol tends to improve wine quality, while volatile acidity worsens it. The outliers appear spread throughout the data, which might suggest that they are not truly extreme—but this is mainly due to our limited ability to perceive structure in high-dimensional spaces.  When we examine the principal components, the picture becomes clearer. Here is the code used to compute them:
...
# Standard-scale the numerical features (exclude target and label columns)
features = df.drop(['quality', 'outliers', 'quality_cat'], axis=1)
X_scaled = scaler.fit_transform(features)

# Project the scaled data onto the first two principal components
pca_data = pca.fit_transform(X_scaled)

# Convert the result to a DataFrame and name the component columns
pca_data = pd.DataFrame(pca_data, columns=['PCA1', 'PCA2'])

# Append the categorical labels for visualisation
pca_data['quality_cat'] = df['quality_cat']
pca_data['outliers'] = df['outliers']
```

Let’s visualise the scatterplot of the first two principal components in relation to wine quality and outlier status:

```python
# Principal component scores for the first two components
PC1 = pca_data.iloc[:, 0]  # first principal component
PC2 = pca_data.iloc[:, 1]  # second principal component

# Scaling factors to bring the PC scores to a comparable range
...
# Names of the original features (used for labelling the arrows)
feature_names = df.drop(['quality', 'outliers', 'quality_cat'], axis=1).columns
...
# Scatter plot of the (scaled) PC scores, coloured by quality and marking outliers
scatter_custom(
    x=pca_data["PCA1"] * scalePC1,
    y=pca_data["PCA2"] * scalePC2,
    data=pca_data,
...
The outliers are now better positioned—far from the main cloud of data points. The arrows clearly show which variables contribute positively or negatively to wine quality.  To create a plot that displays a quality gradient, we first define a colour palette:
...
# Sample six colours from the map and convert them to hex strings
quality_palette = {
...
It would be very interesting to examine the direction of maximal increase in `quality`. At first glance, this direction appears to be expressible as a combination of `alcohol` and `citric acid` within the space spanned by the principal components.  Treating `quality` as a function of the first two principal components:
...
```python
# Build a PCA dataframe that includes the original quality score
pca_data_full = pca_data.copy()
pca_data_full["quality"] = df["quality"]

# Remove the outliers
pca_data_no_out = pca_data_full[pca_data_full["outliers"] == 1]

# Prepare the feature matrix (PC coordinates) and target vector (quality)
X = pca_data_no_out[["PCA1", "PCA2"]]
y = pca_data_no_out["quality"].astype(float)
...
# Regression coefficients represent the gradient of quality w.r.t. the PCs
grad = model.coef_

# Compute the mean loading length (used to scale the gradient vector)
load_lengths = np.sqrt(ldngs[0, :]**2 + ldngs[1, :]**2)
mean_loading_len = load_lengths.mean()

# Normalize the gradient and align its scale with the loadings
...
```python
# Create a separate dataframe with only the outliers
pca_data_out = pca_data_full[pca_data_full["outliers"] == -1]
...
# Plot the inlier points, coloured by wine quality
scatter_custom(
    x=pca_data_no_out["PCA1"] * scalePC1,
    y=pca_data_no_out["PCA2"] * scalePC2,
    alpha=0.8,
...
scatter_custom(
    x=pca_data_out["PCA1"] * scalePC1,
    y=pca_data_out["PCA2"] * scalePC2,
    data=pca_data_out,
    hue=pca_data_out["outliers"],                   # required by scatter_custom
    palette={1: "purple", -1: "purple"},           # force purple colour
    axes=ax,
)

# Draw arrows for each original feature (loadings)
for i, feature in enumerate(feature_names):
    ax.arrow(
        0, 0,
        ldngs[0, i], ldngs[1, i],
...
# Colour bar for the quality values
...
```python
# Prepare a DataFrame with the loadings for each original feature
features_component = pd.DataFrame(ldngs).T
features_component.index = feature_names
features_component.columns = ['x', 'y']

# Function to compute the angle (in radians) between the quality gradient
# direction and a feature loading vector
def angle_between(vec):
    cos_theta = np.dot(grad_dir, vec) / np.linalg.norm(vec)
...
feat_angles = pd.DataFrame({'angle': list_angles, 'feature': feature_names})
feat_angles['angle'] = (feat_angles['angle'] / np.pi) * 180  # convert rad to deg

# Continuous palette from good (small angle) to bad (large angle)
angle_cmap = mcolors.LinearSegmentedColormap.from_list(
    "quality_cmap",
